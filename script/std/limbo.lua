...
local function release(limbo)
  ...
  local connect = putf({ 100, r = 1}, server.N_CONNECT, limbo.reqnick, limbo.playermodel, limbo.password, limbo.authdesc, limbo.authname)
...
spaghetti.addhook(server.N_CONNECT, function(info)
  ...
  engine.sendpacket(info.ci.clientnum, 1, putf({ 1, r = 1}, server.N_WELCOME):finalize(), -1)
