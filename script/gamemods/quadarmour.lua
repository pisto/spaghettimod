...
local fp, L, putf, playermsg, sound = require"utils.fp", require"utils.lambda", require"std.putf", require"std.playermsg", require"std.sound"
...
local function on(probability, divisor, armour, duration, maxduration, decayarmourtype, decayarmour)
  ...
  hooks.pickup = spaghetti.addhook("prepickup", function(info)
    ...
    engine.sendpacket(-1, 1, putf({4, r = 1}, server.N_ITEMACC, info.i, -1):finalize(), -1)
